window.PriceUtils =
  ...
  (() => {
    ...
    const publisherFee = (rgItem, g_rgWalletInfo, defaultFeeValue = false) => {
      ...
      return (
        market_fee || (description && description.market_fee) || wallet_publisher_fee_percent_default || defaultFeeValue
      );
    };
    ...
    const steamBuyerPrice = (receivedAmount, publisherFee, g_rgWalletInfo) => {
      ...
      const nSteamFee = parseInt(
        Math.floor(
          Math.max(receivedAmount * parseFloat(wallet_fee_percent), wallet_fee_minimum) + parseInt(wallet_fee_base)
        )
      );
