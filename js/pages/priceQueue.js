...
var PriceQueue = {
    ...
    },
    {
      ...
      getprice: function (appid, countryCode, currencyId, market_hash_name) {
        ...
        var url =
          ...
          '/' +
          market_hash_name +
          '/render/?start=0&count=5&country=' +
          countryCode +
            ...
            if (response && response.success && response.listinginfo) {
              ...
              if (v_currencyformat && GetCurrencyCode) {
                if (response.listinginfo.length) {
                  ...
                  _response.lowest_price = v_currencyformat(
                    lstItem.converted_price + lstItem.converted_fee,
  ...
  StartQueue: function () {
    ...
    if (PriceQueue._urls.length > 0) {
      ...
      PriceQueue.PricesProviders[PriceQueue._currentproviderIdx].getprice(
        appid,
        countryCode,
        currencyID,
        marketHashName
  ...
  UpdateLabels: function () {
    if (PriceQueue._urls.length == 0) {
      ...
      if (m && m.length > 1) {
        ...
        hashname = hashname.toLocaleLowerCase().indexOf('script') === -1 ? hashname : '';
      }
      $('#_priceQueueCont .pq-info').html(
  ...
  GenPriceDescription: function (rgItems) {
    ...
    var marketLink =
      ...
      rgItems.appid +
      '/' +
      encodeURIComponent(rgItems.market_hash_name);

    var ddHtml =
      priceProvider +
      ": <a href='" +
      marketLink +
      "' target='_blank' style='color:#FF0' title='" +
      rgItems.nofeePrice +
      "'>" +
      rgItems.lowestPrice;
    if (rgItems.volume) {
  ...
  _generalHandler: {
    success: function (response, url, providerName) {
      ...
      if (PriceQueue._successDelay < 3000) {
        PriceQueue._successDelay = 3000;
      }

      $('#_priceQueueCont .pq-progress')
        .stop()
        .css({ width: '1%' })
    ...
    error: function (isAppError) {
      ...
      if (PriceQueue._currentproviderIdx >= PriceQueue.PricesProviders.length) {
        if (PriceQueue._failureDelay < 30000) {
          PriceQueue._failureDelay = 30000;
        }
