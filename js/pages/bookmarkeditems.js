...
const renderCategoriesSelector = ($container, categoriesData) => {
  ...
  renderCategoriesSelectOptions($('#cb_bookmarkscategories'), categoriesData.bookmarkscategories);
...
const renderCategoriesSelectOptions = ($select, categories) => {
  ...
  Object.keys(categories).forEach((key) => $select.append(`<option value="${key}">${categories[key]}</option>`));
};
...
const renderItem = ($container, item) => {
  ...
  $row.append(
    '<div class="name">' +
      ...
      '" style="border-color: ' +
      item.color +
      '; width: 38px; height: 38px;" class="market_listing_item_img" alt="">' +
      '<span style="color: ' +
      item.color +
      ';"><a href="' +
      item.url +
  ...
  $row.append(
    '<div class="volume"><span class="bookmark-volume">Loading...</span><br /><span class="bookmark-median-price"></span></div>'
  );
  $row.append(
    '<div class="price"><span class="bookmark-lowest-price">Loading...</span><br /><span class="bookmark-seller-price"></span></div>'
  );
  $row.append(
    '<div class="remove"><span data-hash="' +
      item.hashmarket +
      '" class="remove-bookmark" title="Remove">x</span></div>'
  );
